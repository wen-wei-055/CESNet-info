...
class FFTEmbedding(nn.Module):
    def __init__(self, device, eps=1e-8):
        ...
        self.eps=eps
        self.Conv2D_1 = nn.Conv2d(in_channels=1, out_channels=8, kernel_size=(3, 1), stride=(3, 1))
        self.Conv2D_2 = nn.Conv2d(in_channels=8, out_channels=32, kernel_size=(16, 3), stride=(1, 3))

        self.Conv1D_1 = nn.Conv1d(in_channels=64, out_channels=64, kernel_size=16)
        self.Conv1D_2 = nn.Conv1d(in_channels=64, out_channels=32, kernel_size=8)
        self.Conv1D_3 = nn.Conv1d(in_channels=32, out_channels=16, kernel_size=4)

        self.pool1D_1 = nn.MaxPool1d(kernel_size=2)
        self.pool1D_2 = nn.MaxPool1d(kernel_size=2)
              
    def forward(self, x):  #(249, 500, 6)
        x = torch.unsqueeze(x,1)  #(249, 1, 500, 6) :channel dimension 
        x = F.relu(self.Conv2D_1(x))  #(249, 8, 166, 6)
        x = F.relu(self.Conv2D_2(x))  #(249, 32, 151, 2)
        x = x.contiguous().view(x.shape[0], -1, x.shape[-2])  #(249, 64, 151)
        x = self.pool1D_1(F.relu(self.Conv1D_1(x)))  #(249, 64, 68)
        x = self.pool1D_2(F.relu(self.Conv1D_2(x)))  #(249, 32, 30)
        x = F.relu(self.Conv1D_3(x))  #(249, 16, 27)
        x = torch.flatten(x, 1)  #(249, 432)
        return x


class To_GaussianDistribution(nn.Module):
    def __init__(self, n_pga_targets, emb_dim=500, output_mlp_dims=(150, 100, 50, 30, 10), pga_mixture=5, device='cpu'):
...
class info_weight(nn.Module):
    def __init__(self, device):
        super(info_weight, self).__init__()
        # equal:          (8, 249, 249)   # 3,4,5-,5+,6-,6+,7,total 
        # near(相差一級):  (5, 249, 249)    # 4,5-,5+,6-,6+
        # distances:      (249, 249)
        
        tmp1 = torch.from_numpy(np.load("information_data/match_relation_table.npy").astype('float32')).to(device)
        history_pga_equally = tmp1 / torch.max(tmp1)
        self.history_pga_equally = history_pga_equally.permute(1,2,0) #(249,249,8)
        
        tmp2 = torch.from_numpy(np.load("information_data/dist_matrix.npy").astype('float32')).to(device)
        self.distances = tmp2 / torch.max(tmp2)

        self.dis1 = nn.Parameter(torch.ones(249,249).to(device))
        self.his1 = nn.Parameter(torch.ones(8,1).to(device))
        self.his2 = nn.Parameter(torch.ones(249,249).to(device))
        
        self.learned_info = nn.Parameter(torch.zeros(249,249).to(device))
        self.FFTEmbedding = FFTEmbedding(device=device)
        self.FFTTimeDistributed = TimeDistributed(self.FFTEmbedding)
        
        self.MLP = MLP((249,), (1,))
        
    def forward(self, x, fft_waveforms):
        fft_waveforms = self.FFTTimeDistributed(fft_waveforms)
        fft_waveforms = F.relu(torch.matmul(fft_waveforms, fft_waveforms.permute(0,2,1)))
        dis_emb = F.relu(torch.matmul(self.distances, self.dis1)) #(249, 249)
        his_emb = F.relu(torch.matmul(self.history_pga_equally, self.his1)).squeeze(-1) #(249, 249)
        his_emb = F.relu(torch.matmul(his_emb, self.his2)) #(249, 249)
        
        dis_emb = F.relu(torch.matmul(fft_waveforms, dis_emb)) #(batch, 249, 249)
        total_emb = F.relu(torch.matmul(dis_emb, his_emb)) #(batch, 249, 249)
        total_emb = torch.sigmoid(self.MLP(total_emb))
        x = x * total_emb
        return x
...
class FullModel(nn.Module):
    def __init__(self, device, input_shape, downsample, mlp_dims, wavelength, left_station, right_station,
        ...
        self.info_weight = info_weight(device=device)
        
        self.To_GaussianDistribution = To_GaussianDistribution(n_pga_targets=n_pga_targets, emb_dim=mlp_dims[-1], 
                                                               output_mlp_dims=output_mlp_dims, pga_mixture=pga_mixture, device=device)

    def forward(self, waveforms, coords, survival_list, stations_channel_class, fft_waveforms):
        ...
        x = self.info_weight(x, fft_waveforms)
...
def fft_generator(xs, device, cutout):
    dim0, dim1, dim2, dim3 = xs.shape
    hidden_dim = 500 
    
    xs = xs.permute(0,1,3,2)
    xs = xs.contiguous().view(-1, dim2) #(batch, station, 3000, 3)
    output_mag = torch.zeros((xs.shape[0], hidden_dim)).to(device)
    output_angle = torch.zeros((xs.shape[0], hidden_dim)).to(device)
    
    selected_indices = torch.nonzero(torch.any(xs != 0, dim=1).int(), as_tuple=False).squeeze(dim=1)  #(batch*station*3, 3000)
    if selected_indices.shape[0] != 0:
        xs_select = xs[selected_indices][:,:cutout]
        
        xs_select = torch.fft.fft(xs_select, n=hidden_dim) / 50
        output_mag[selected_indices] = torch.abs(xs_select)
        output_angle[selected_indices] = torch.angle(xs_select) / torch.pi

        output_mag = output_mag.contiguous().view(dim0, dim1, dim3, hidden_dim)  #(batch, station, 3, 500)
        output_angle = output_angle.contiguous().view(dim0, dim1, dim3, hidden_dim)  #(batch, station, 3, 500)
        
        output_mag = output_mag.permute(0,1,3,2).to(device)
        output_angle = output_angle.permute(0,1,3,2).to(device)
    else:
        output_mag = torch.zeros((dim0, dim1, hidden_dim, dim3)).to(device)
        output_angle = torch.zeros((dim0, dim1, hidden_dim, dim3)).to(device)
        output_mag = output_mag + 1e-10
        output_angle = output_angle + 1e-10
...
class EnsembleEvaluateModel:
    ...
    def predict_generator(self, generator, **kwargs):
        ...
        no_survival_preds = torch.tensor([], device=self.device)
        ...
        for i, model in enumerate(self.models):
            ...
            tmp_no_survival_preds = torch.tensor([], device=self.device)  

            model.eval()
            with torch.no_grad():
                for x,y,cutout in dataloader: 
                    cutout = cutout[1]
                    ...
                    fft_mag_waveforms, fft_angle_waveforms = fft_generator(inputs_waveforms, self.device, cutout)
                    fft_waveforms = torch.cat((fft_mag_waveforms,fft_angle_waveforms), -1)

                    survival_list = survival_list[:,-249:]
                    
                    outputs = model(inputs_waveforms, inputs_coords, survival_list, None, fft_waveforms)  
                    outputs =  torch.squeeze(outputs[:,-249:], 0)
                    
                    no_survival_outputs = outputs.clone()
                    tmp_no_survival_preds = torch.cat((tmp_no_survival_preds, torch.unsqueeze(no_survival_outputs,0)), 0)
            ...
            tmp_no_survival_preds = torch.unsqueeze(tmp_no_survival_preds, 0)

            preds = torch.cat((preds, tmp_preds), 0) 
            no_survival_preds = torch.cat((no_survival_preds, tmp_no_survival_preds), 0) 
            
        preds = preds.cpu().numpy()
        no_survival_preds = no_survival_preds.cpu().numpy()

        return self.merge_preds(preds), self.merge_preds(no_survival_preds)
