...
def calculate_warning_times(config, model_list, data, event_metadata, batch_size, sampling_rate=100,
    ...
    for i, _ in tqdm(enumerate(event_metadata.iterrows()), total=len(event_metadata)): 
        ...
        predictions, no_survival_predictions = model_list.predict_generator(cutout_generator, workers=0, use_multiprocessing=False)  
        
        predictions = [predictions, no_survival_predictions]
        for prediction_idx, prediction in enumerate(predictions):
            ...
            for station_index in range(coords.shape[0]):
                ...
                station_key = '{},{},{}'.format(station[0], station[1], station[2])
            ...
            if prediction_idx == 0:
                ...
                no_survival_full_predictions += [(pga_times_pred, pga_times_true, dist)]

    return full_predictions, no_survival_full_predictions
...
if __name__ == '__main__':
    ...
    if args.head_times:
        ...
        warning_time_information, no_survival_warning_time_information = calculate_warning_times(config, model_list, data, event_metadata,
                                                           times=times_pga,
                                                           alpha=alpha,
    ...
    with open(os.path.join(output_dir, '08_EndSurvival_warning.pkl'), 'wb') as pred_file:
        pickle.dump((times, mag_pred_full, loc_pred_full, pga_pred_full, warning_time_information, alpha), pred_file)
        

    with open(os.path.join(output_dir, '08_EndSurvival_no_survival_warning.pkl'), 'wb') as pred_file:
        pickle.dump((times, mag_pred_full, loc_pred_full, pga_pred_full, no_survival_warning_time_information, alpha), pred_file)
