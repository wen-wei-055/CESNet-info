...
def training(model, optimizer, loader, epoch, epochs, device, training_params, pga_loss, train_loss_record, logger):
    ...
    for x,y,cutout in train_loop:
        ...
        pred = model(inputs_waveforms, inputs_coords, survival_list, stations_channel_class, fft_waveforms) 
...
def validating(model, optimizer, loader, epoch, epochs, device, pga_loss, scheduler, val_loss_record, logger):
    ...
    confusion_matrix = np.zeros((len(thresholds)+1, len(thresholds)+1)).astype(np.int32)
    with torch.no_grad():
        for x,y,coutout in valid_loop:
            inputs_waveforms, inputs_coords, survival_list, targets_pga = \
                    x[0].to(device), x[1].to(device), x[2].to(device).long(), y[0].to(device)

            fft_mag_waveforms, fft_angle_waveforms = fft_generator(inputs_waveforms, device, coutout)
            fft_waveforms = torch.cat((fft_mag_waveforms,fft_angle_waveforms), -1)
...
if __name__ == '__main__':
    ...
    for ens_id in [0]:
        ...
        full_model = freeze_model(models.build_transformer_model(**config['model_params'], device=device, trace_length=3000), ['TotalEmbedding', 'Transformer', 'To_GaussianDistribution']).to(device)
        ...
        for epoch in range(num_epochs):
            ...
            #save model
            if (epoch>=1) and (val_loss_record[-1] < min(val_loss_record[:-1])) or epoch%9==0: 
